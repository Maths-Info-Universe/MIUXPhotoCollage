 ...
 */
public abstract class MIUDialogModel extends JDialog{
    ...
    protected MIUButton okButton, cancelButton, nextButton, prevButton;
    protected int commandType = 1;
    protected boolean showNextPrevKeys = false, focusOnAction = true;
    ...
    public MIUDialogModel (MIUConfigManager config, Frame owner, String title, boolean modal, boolean showNextPrevKeys){
    ...
    public MIUDialogModel (int commandType, MIUConfigManager config, Frame owner, String title, boolean modal, boolean showNextPrevKeys){
        ...
        steps = new ArrayList<>();
        MIUDialogModel.config = config;
        initPanel();
    }
    
    public MIUDialogModel (MIUConfigManager config, Frame owner, String title, boolean modal, boolean showNextPrevKeys, boolean focusOnAction){
    ...
    public MIUDialogModel (MIUConfigManager config, Dialog owner, String title, boolean modal, boolean showNextPrevKeys){
        this(OK_CANCEL_BUTTON, config, owner, title, modal, showNextPrevKeys);
    }
    ...
    public MIUDialogModel (MIUConfigManager config, Dialog owner, String title, boolean modal, boolean showNextPrevKeys, boolean focusOnAction){
        this(OK_CANCEL_BUTTON, config, owner, title, modal, showNextPrevKeys, focusOnAction);
    }
    ...
    private void initPanel(){
        ...
        menuPanel.add(lab);
        if(showNextPrevKeys){
            prevButton = new MIUButton(config.getLangValue(PREV_KEY), config.getTheme());
            ...
            nextButton = new MIUButton(config.getLangValue(NEXT_KEY), config.getTheme());
        ...
        if(commandType != CANCEL_BUTTON){
            okButton = new MIUButton(config.getLangValue(OK_KEY), config.getTheme());
        ...
        if(commandType != OK_BUTTON){
            cancelButton = new MIUButton(config.getLangValue(CANCEL_KEY), config.getTheme());
        ...
        stepPanel = new MIUStyledPanel(MIUStyledPanel.SIMPLE_CHRISTMAS, config);
        ...
        keyListener = new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                if(e.getKeyCode() == 10){
                    ...
                    if((e.getSource() instanceof MIUButton)){
    ...
    public void addStep(JLabel step){
        ...
        step.setToolTipText("<html><body><font family=\""+config.getTheme().getToolTipFont().getFamily()+"\" size=\""+config.getTheme().getToolTipSize()+"\" color=\""+MIUTheme.extractRGB(config.getTheme().getToolTipColor())+"\">"
    ...
    public void setStep(int step) throws MIUApplException{
        if(step > 0 && step <= steps.size()){
            ...
            try{
                ...
                throw new MIUApplException(ex.getMessage(), ex);
            }
